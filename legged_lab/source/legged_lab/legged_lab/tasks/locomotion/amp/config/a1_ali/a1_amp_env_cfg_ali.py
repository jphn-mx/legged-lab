...
class A1AmpEnvCfg(LocomotionAmpEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.motion_data.motion_dataset.motion_data_weights = {
            ...
            # ---- Curated for command vx in [0.6, 1.0], no backward, no run, no <0.3 slow ----
            # Disc obs are body-local (imitate GAIT not speed), so steady walk + walk-turns cover
            # the 0.6-1.0 forward gait; run / low-speed-transition / backward clips removed since
            # they are now off the commanded manifold.
            # forward steady walk (only clean walk clip -> weight 2.0)
            "B3_-_walk1": 2.0,
            # walk turns ~0.5 wz (also reinforce the forward-walk gait)
            "B9_-__Walk_turn_left_90": 1.0,
            "B13_-__Walk_turn_right_90": 1.0,
            "B11_-__Walk_turn_left_135": 1.0,
            "B15_-__Walk_turn_around": 1.0,
