...
class A1RslRlOnPolicyRunnerAmpCfg(RslRlOnPolicyRunnerCfg):
    ...
    algorithm = RslRlPpoAmpAlgorithmCfg(
        ...
        amp_cfg=RslRlAmpCfg(
            ...
            disc_max_grad_norm=1.0,
            amp_discriminator=RslRlAmpCfg.AMPDiscriminatorCfg(
                hidden_dims=[1024, 512], activation="elu", style_reward_scale=2.0, task_style_lerp=0.6,
...
@configclass
class A1RslRlOnPolicyRunnerAmpFixedCfg(RslRlOnPolicyRunnerCfg):
    """Dedicated AMP runner for the fixed-speed + stand-still A1 task.

    Standalone (not inherited) so it can be tuned independently of the
    range-tracking A1 AMP config. Logs to its own experiment folder.
    """
    ...
    algorithm = RslRlPpoAmpAlgorithmCfg(
        ...
        amp_cfg=RslRlAmpCfg(
            ...
            amp_discriminator=RslRlAmpCfg.AMPDiscriminatorCfg(
                ...
                hidden_dims=[1024, 512], activation="elu", style_reward_scale=2.0, task_style_lerp=0.4,
...
@configclass
class A1RslRlOnPolicyRunnerAmpRangeCfg(RslRlOnPolicyRunnerCfg):
    """Dedicated AMP runner for the fixed-speed + stand-still A1 task.
...
@configclass
class A1RslRlOnPolicyRunnerAmpWalkCfg(A1RslRlOnPolicyRunnerAmpRangeCfg):
    """Runner for the standalone walk/turn range task. Same algo params as the range
    runner, own experiment (log) folder."""

    experiment_name = "a1_amp_walk"
