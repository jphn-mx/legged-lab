...
parser.add_argument("--robot", type=str, default="g1", choices=["g1", "a1"], help="The robot to use.")
...
from whole_body_tracking.robots.a1 import A1_LEGS_CFG
...
selected_robot = ROBOT_CONFIGS[args_cli.robot]
...
class ReplayMotionsSceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    robot: ArticulationCfg = selected_robot["cfg"].replace(prim_path="{ENV_REGEX_NS}/Robot")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        if reset_flag and not file_saved:
            ...
            output_path = os.path.join(MOTIONS_DIR, f"{args_cli.output_name}.npz")
...
def main():
    ...
    run_simulator(
        sim,
        scene,
        joint_names=selected_robot["joint_names"],
    )
